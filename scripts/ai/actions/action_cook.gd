...
func can_execute(villager: Villager, world: Node) -> bool:
	...
	if not BuildingManager.has_building("campfire"):
		return false
	var total: int = villager.inventory.get_amount("raw_meat") + BuildingManager.get_storage_items("raw_meat")
	return total >= 1

func calculate_utility(villager: Villager, world: Node) -> float:
	if not can_execute(villager, world):
		return 0.0
	var meat_count: int = villager.inventory.get_amount("raw_meat") + BuildingManager.get_storage_items("raw_meat")
...
func start(villager: Villager, world: Node) -> void:
	...
	if villager.inventory.has_item("raw_meat", 1):
	...
	match _phase:
		Phase.FETCHING:
			if villager.has_reached_target():
				BuildingManager.withdraw_items(villager, {"raw_meat": 1})
